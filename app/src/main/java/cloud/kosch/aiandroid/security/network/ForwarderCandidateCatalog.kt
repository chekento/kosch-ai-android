...
object ForwarderCandidateCatalog {
     ...
     * dialer exist in the pinned source. The exported Engine.Start path uses fatal logging on start
     * error, so recoverable KoSch start-failure handling is not yet verified. No KoSch Android
     * embedding/protect/lifecycle evidence exists yet.
     */
    val TUN2SOCKS_2_7_0 = ForwarderCandidateProfile(
        ...
        recoverableStartFailureVerified = false,
     ...
     * Its C start API returns an error status and its quit API is explicit, but KoSch still has no
     * protect/no-black-hole/OEM evidence.
     */
